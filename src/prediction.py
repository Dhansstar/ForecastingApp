...
def run():
    ...
    fig = make_subplots(rows=2, cols=1, vertical_spacing=0.15, specs=[[{"type": "table"}], [{"type": "scatter"}]], row_heights=[0.3, 0.7])
    ...
    fig.add_trace(go.Table(
        ...
        cells=dict(values=[[f"<b>{row_data['Kategori']}</b>"], [row_data['Horizon']], [f"{row_data['MAPE (%)']:.2f}%"], [row_data['MAE']], [f"{row_data['Vol Acc (%)']:.2f}%"], [int(row_data['Safety Stock'])], [int(row_data['Total Procurement'])]],
                    fill_color='rgba(0,0,0,0.5)', font=dict(color='white', size=11), align='center')
    ), row=1, col=1)

    fig.add_trace(go.Scatter(x=df_hist['Waktu Pesanan Dibuat'], y=df_hist['Net_Sales'], name='Actual (Last 30D)', line=dict(color='#7f8c8d', width=2)), row=2, col=1)
    fig.add_trace(go.Scatter(x=conn_dates, y=conn_values, name=f'Forecast {target_hz}', line=dict(color='#3498db', width=3, dash='dash')), row=2, col=1)
    ...
    fig.add_trace(go.Scatter(x=conn_dates + conn_dates[::-1], y=[y + daily_safe for y in conn_values] + [max(0, y - daily_safe) for y in conn_values][::-1],
                             fill='toself', fillcolor='rgba(231, 76, 60, 0.1)', line=dict(color='rgba(255,255,255,0)'), name='Safety Stock Margin'), row=2, col=1)
    ...
    fig_bar = px.bar(current_hz_data, x="Kategori", y=["Est. Demand", "Safety Stock"], barmode="group", color_discrete_sequence=['#3498db', '#e74c3c'], template="plotly_dark")
